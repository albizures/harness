...
test("help returns a stable success envelope", async () => {
  ...
  const data = envelope.data as { name: string; description: string; commands: Array<{ usage: string }> };
...
test("envelopes serialize as one JSON stdout line", () => {
  assert.equal(serializeEnvelope({ ok: true, data: { smoke: true } }), '{"ok":true,"data":{"smoke":true}}\n');
});

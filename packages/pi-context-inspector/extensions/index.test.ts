...
function makeCommandContext(overrides: Record<string, unknown> = {}) {
  return {
    ...
    isIdle: () => false,
    getContextUsage: () => ({ tokens: 51200, contextWindow: 128000, percent: 40 }),
    getSystemPrompt: () => [
    ...
    getSystemPromptOptions: () => ({
      ...
      skills: [{ name: "implement", description: "Implement work", content: "Use TDD" }],
    }),
    sessionManager: {
      buildContextEntries: () => [
        { type: "message", message: { role: "user", content: [{ type: "text", text: "Please inspect context" }] } },
        { type: "message", message: { role: "assistant", content: [{ type: "text", text: "Sure" }] } },
        { type: "message", message: { role: "toolResult", toolName: "read", content: [{ type: "text", text: "file output" }] } },
      ],
    },
    ui: {
      notify: () => {},
...
test("registers /context-inspector with the required description", () => {
  ...
  assert.equal(command.description, "Inspect estimated Pi context usage by source.");
});
...
test("collectContextInspectorInputs captures model, usage, prompt options, and active context entries", () => {
  ...
  assert.deepEqual(inputs.model, { id: "gpt-test", provider: "openai", contextWindow: 128000 });
  assert.deepEqual(inputs.contextUsage, { tokens: 51200, contextWindow: 128000, percent: 40 });
  assert.match(inputs.systemPrompt, /Base Pi system prompt text/);
  assert.equal(inputs.systemPromptOptions.contextFiles?.[0]?.path, "/repo/AGENTS.md");
...
test("estimateContextAttribution uses canonical bucket labels and contributor names without raw snippets", () => {
  const estimate = estimateContextAttribution(collectContextInspectorInputs(makeCommandContext()));

  assert.ok(estimate.buckets.some((bucket) => bucket.name === "System prompt"));
  assert.ok(estimate.buckets.some((bucket) => bucket.name === "Tool definitions"));
  ...
  assert.ok(estimate.buckets.some((bucket) => bucket.name === "Assistant messages"));
  assert.ok(estimate.buckets.some((bucket) => bucket.name === "Tool results"));

  const contextFiles = estimate.buckets.find((bucket) => bucket.name === "Context files")!;
  assert.equal(contextFiles.contributors[0]?.name, "/repo/AGENTS.md");
  assert.doesNotMatch(JSON.stringify(estimate), /Project guidance|Read files|Use TDD/);
});
...
test("showContextInspectorReport opens a read-only scrollable TUI overlay", async () => {
  ...
  const ctx = makeCommandContext({
    ...
    hasUI: true,
    ui: {
      notify: () => assert.fail("TUI mode with UI should use the overlay, not fallback notification"),
  ...
  assert.equal(initialRender.filter((line: string) => /report line \d+/.test(line)).length, 24);
  ...
  assert.match(lastPageRender, /│ esc\/enter closes • ↑↓ scroll .* 7-30\/30\s+│/);
  ...
  const ctx = makeCommandContext({
    ...
    ui: {
      ...
      custom: async (factory: any) => {
        component = factory(
          { requestRender: () => {} },
          { fg: (color: string, text: string) => color === "accent" ? `<accent>${text}</accent>` : text, bold: (text: string) => text },
          {},
          () => {},
        );
      },
...
test("showContextInspectorReport falls back to a compact notification when full TUI display is unavailable", async () => {
  ...
  await showContextInspectorReport(ctx, {
    ...
    renderTui: () => { throw new Error("fallback should not render TUI report"); },
  });
...
test("renderTuiContextInspectorReport colors only dynamic values", () => {
  const inputs = collectContextInspectorInputs(makeCommandContext({
    getContextUsage: () => ({ tokens: null, contextWindow: undefined, percent: null }),
    model: undefined,
  }));
  const attribution = estimateContextAttribution(inputs);

  const plain = renderContextInspectorReport(inputs, attribution);
  const styled = renderTuiContextInspectorReport(inputs, attribution, (value) => `<accent>${value}</accent>`);

  assert.doesNotMatch(plain, /<accent>/);
  assert.match(styled, /Collected: <accent>[^<]+ \(active at collection time\)<\/accent>/);
  ...
  assert.match(styled, /Estimated sum of visible sources: <accent>\d+ tokens<\/accent>/);
  assert.match(styled, /- System prompt: <accent>~\d+ tokens<\/accent> <accent>\(\d+\.\d%\)<\/accent>/);
...
test("renderContextInspectorReport shows known Pi total and frames attribution as estimated", () => {
  const report = renderContextInspectorReport(
    ...
    estimateContextAttribution(collectContextInspectorInputs(makeCommandContext())),
  );
...
test("known Pi total scales bucket estimates and shares use the authoritative total", () => {
  ...
  assert.deepEqual(estimate.buckets.map((bucket) => [bucket.name, bucket.estimatedTokens, bucket.sharePercent]), [
...
test("unknown total leaves estimates unscaled and shares use the unscaled estimate total", () => {
  ...
  assert.match(report, /Pi total is unknown; source shares use the unscaled visible-source estimate as their denominator\./);
});
...
test("zero buckets are hidden and nonzero buckets are sorted by estimated token count descending", () => {
  const inputs = collectContextInspectorInputs(makeCommandContext({
    ...
    sessionManager: {
      buildContextEntries: () => [
        { type: "message", message: { role: "user", content: "A".repeat(4) } },
      ],
    },
  }));
  ...
  assert.deepEqual(estimate.buckets.map((bucket) => [bucket.name, bucket.estimatedTokens]), [
    ["Skills", 4],
    ["Context files", 2],
    ["User messages", 1],
  ]);
  assert.equal(estimate.buckets.some((bucket) => bucket.estimatedTokens === 0), false);
  assert.equal(estimate.buckets.some((bucket) => bucket.name === "System prompt"), false);
});
...
test("System prompt attribution excludes prompt sources with dedicated buckets", () => {
  ...
  const inputs = collectContextInspectorInputs(makeCommandContext({
    ...
    getSystemPromptOptions: () => ({
      ...
      contextFiles: [{ path: "/repo/CONTEXT.md", content: `context file ${marker}` }],
  ...
  const estimate = estimateContextAttribution(inputs);
  const byName = Object.fromEntries(estimate.buckets.map((bucket) => [bucket.name, bucket]));

  assert.ok(byName["System prompt"].estimatedTokens < byName["Tool definitions"].estimatedTokens);
  assert.ok(byName["System prompt"].estimatedTokens < byName["Context files"].estimatedTokens);
  assert.ok(byName["System prompt"].estimatedTokens < byName["Skills"].estimatedTokens);
  assert.deepEqual(byName["System prompt"].contributors.map((item) => item.name).sort(), [
...
test("System prompt bucket is omitted when all prompt text belongs to dedicated files, skills, and tools", () => {
  ...
  const inputs = collectContextInspectorInputs(makeCommandContext({
    getSystemPrompt: () => ["tool " + marker, "context " + marker, "skill " + marker].join("\n"),
    getSystemPromptOptions: () => ({
      cwd: "/repo",
      selectedTools: ["read"],
      toolSnippets: { read: "tool " + marker },
      contextFiles: [{ path: "/repo/CONTEXT.md", content: "context " + marker }],
...
test("Tool definitions ignore snippets for tools that are not active", () => {
  ...
  const estimate = estimateContextAttribution(inputs);
  const tools = estimate.buckets.find((bucket) => bucket.name === "Tool definitions")!;

  assert.deepEqual(tools.contributors.map((item) => item.name), ["read"]);
...
test("Tool definitions use active tool names and report caveats about provider schemas", () => {
  ...
  const estimate = estimateContextAttribution(inputs);
  const tools = estimate.buckets.find((bucket) => bucket.name === "Tool definitions")!;
  assert.deepEqual(tools.contributors.map((item) => item.name).sort(), ["customTool", "read"]);

  const report = renderContextInspectorReport(inputs, estimate);
  assert.match(report, /Tool definitions are estimated from prompt-visible names and snippets, not provider-serialized schemas\./);
...
test("conversation attribution uses only active branch context entries", () => {
  const inputs = collectContextInspectorInputs(makeCommandContext({
    sessionManager: {
      buildContextEntries: () => [
        { type: "message", message: { role: "user", content: "active user request" } },
        { type: "message", message: { role: "assistant", content: [{ type: "text", text: "active assistant reply" }] } },
        { type: "message", message: { role: "toolResult", toolName: "read", content: [{ type: "text", text: "active tool output" }] } },
      ],
      getEntries: () => [
        { type: "message", message: { role: "user", content: "abandoned branch user content" } },
        { type: "message", message: { role: "assistant", content: [{ type: "text", text: "abandoned branch assistant content" }] } },
      ],
    },
  }));
  ...
  assert.doesNotMatch(JSON.stringify(estimate), /abandoned branch|active user request|active assistant reply|active tool output/);
});

test("compaction attribution counts summary text and retained active messages, not token metadata", () => {
  const inputs = collectContextInspectorInputs(makeCommandContext({
    getContextUsage: () => ({ tokens: null, contextWindow: 128000, percent: null }),
    sessionManager: {
      buildContextEntries: () => [
        {
          type: "compaction",
          ...
          retainedTail: [
            ...
            { role: "assistant", content: [{ type: "text", text: "retained assistant text" }] },
          ],
        },
      ],
    },
  ...
  assert.equal(byName["Compactions and summaries"].contributors[0]?.name, "compaction");
  assert.ok(byName["User messages"].estimatedTokens > 0);
  assert.ok(byName["Assistant messages"].estimatedTokens > 0);
  assert.doesNotMatch(JSON.stringify(estimate), /100000|retained user text|retained assistant text|short remaining summary/);
});
...
test("custom context messages are attributed to other active entries with safe custom labels", () => {
  const inputs = collectContextInspectorInputs(makeCommandContext({
    sessionManager: {
      buildContextEntries: () => [
        { type: "custom_message", customType: "extension\nraw custom label", content: "custom message body" },
      ],
    },
  }));

  const estimate = estimateContextAttribution(inputs);
  const other = estimate.buckets.find((bucket) => bucket.name === "Other active entries")!;

  assert.equal(other.contributors[0]?.name, "extension");
  assert.doesNotMatch(JSON.stringify(estimate), /raw custom label|custom message body/);
});
...
test("message and tool-result contributors use labels without raw content snippets", () => {
  const inputs = collectContextInspectorInputs(makeCommandContext({
    sessionManager: {
      buildContextEntries: () => [
        { type: "message", message: { role: "user", content: "secret user snippet" } },
        { type: "message", message: { role: "assistant", content: [{ type: "text", text: "secret assistant snippet" }] } },
        { type: "message", message: { role: "toolResult", toolName: "bash\nsecret tool label", content: [{ type: "text", text: "secret tool output" }] } },
        { type: "message", message: { role: "toolResult", content: [{ type: "text", text: "anonymous tool output" }] } },
      ],
    },
  }));
  ...
  assert.doesNotMatch(serialized, /secret user snippet|secret assistant snippet|secret tool output|secret tool label|anonymous tool output/);
});
...
test("rendered contributor lists are capped at five and shown only for materially large buckets", () => {
  ...
  const inputs = collectContextInspectorInputs(makeCommandContext({
    getContextUsage: () => ({ tokens: null, contextWindow: 128000, percent: null }),
    getSystemPrompt: () => contextFiles.map((file) => file.content).join("\n") + "\nsmall user note",
    getSystemPromptOptions: () => ({ cwd: "/repo", selectedTools: [], contextFiles }),
    sessionManager: { buildContextEntries: () => [{ type: "message", message: { role: "user", content: "small user note" } }] },
  }));

  const report = renderContextInspectorReport(inputs, estimateContextAttribution(inputs));
...
test("recommendations are read-only plain text for large context files, skills, and tool results", () => {
  const inputs = collectContextInspectorInputs(makeCommandContext({
    ...
    getSystemPromptOptions: () => ({
      cwd: "/repo",
      selectedTools: [],
      contextFiles: [{ path: "/repo/LARGE.md", content: "context file ".repeat(500) }],
      skills: [{ name: "large-skill", content: "skill body ".repeat(500) }],
    }),
    sessionManager: {
      buildContextEntries: () => [
        { type: "message", message: { role: "toolResult", toolName: "bash", content: "tool output ".repeat(500) } },
      ],
    },
  }));
  ...
  assert.match(report, /Large tool results: review whether bulky outputs still need to remain in active context/);
  assert.match(report, /Large context files: consider whether the current project context can be narrowed/);
  assert.match(report, /Large skills: consider whether every loaded skill is relevant to the current task/);
  assert.doesNotMatch(report, /button|click|trigger|compact now|edit|apply|prune/i);
  assert.doesNotMatch(report, /context file context file|skill body skill body|tool output tool output/);
});

	...
	pi.registerCommand(COMMAND_NAME, {
		...
		handler: async (_args, ctx) => {
			...
			const report = {
				...
				renderTui: (colorValue: (value: string) => string) =>
					renderTuiContextInspectorReport(inputs, attribution, colorValue),
			};
...
export function collectContextInspectorInputs(
	ctx: ExtensionCommandContext,
): ContextInspectorInputs {
...
export function estimateContextAttribution(
	inputs: ContextInspectorInputs,
): ContextAttributionEstimate {
	const rawBuckets = [
	...
	const rawTotal = rawBuckets.reduce(
		(total, bucket) => total + bucket.estimatedTokens,
		0,
	);
	const authoritativeTotal = knownTotalTokens(inputs.contextUsage);
	const denominator = authoritativeTotal ?? rawTotal;
	const buckets = (
		authoritativeTotal === undefined || rawTotal === 0
			? rawBuckets
	...
	return {
		estimatedTotalTokens: buckets.reduce(
			(total, bucket) => total + bucket.estimatedTokens,
			0,
		),
		buckets,
...
export function renderContextInspectorReport(
	...
	return renderContextInspectorReportWithValueFormatter(
		inputs,
		attribution,
		(value) => value,
	);
...
function renderContextInspectorReportWithValueFormatter(
	...
	const model = inputs.model
		...
		: "unknown";
	const contextWindow =
		inputs.contextUsage?.contextWindow ?? inputs.model?.contextWindow;
	...
	if (attribution.buckets.length === 0) {
		...
		for (const bucket of attribution.buckets) {
			lines.push(
				`- ${bucket.name}: ${formatValue(`~${formatTokens(bucket.estimatedTokens)} tokens`)} ${formatValue(`(${bucket.sharePercent.toFixed(1)}%)`)}`,
			);
			if (isMaterialBucket(bucket)) {
				for (const contributor of bucket.contributors.slice(
					0,
					MAX_CONTRIBUTORS_PER_BUCKET,
				)) {
					lines.push(
						`  - ${contributor.name}: ${formatValue(`~${formatTokens(contributor.estimatedTokens)} tokens`)}`,
					);
				}
			}
		}
...
export async function showContextInspectorReport(
	ctx: ExtensionCommandContext,
	report: string | ContextInspectorReport,
): Promise<void> {
				...
				(tui, theme, _keybindings, done) => {
					...
					return {
						render(width: number): string[] {
							...
							const help = theme.fg(
								"dim",
								`esc/enter closes • ↑↓ scroll • pgup/pgdn jump • read-only • ${position}`,
							);
							return [
								frameBorderLine(width, "top", (text) =>
									theme.fg("accent", text),
								),
								frameContentLine(title, width, (text) =>
									theme.fg("accent", text),
								),
								frameContentLine(help, width, (text) =>
									theme.fg("accent", text),
								),
								...
								...lines
									.slice(
										scrollOffset,
										scrollOffset + OVERLAY_BODY_VIEWPORT_LINES,
									)
									.map((line) =>
										frameContentLine(line, width, (text) =>
											theme.fg("accent", text),
										),
									),
								frameBorderLine(width, "bottom", (text) =>
									theme.fg("accent", text),
								),
							];
						},
	...
	if (ctx.hasUI) {
		ctx.ui.notify(
			"Context Inspector: report ready (open in TUI mode for the scrollable view).",
			"info",
		);
		return;
	}
...
function frameContentLine(
	...
	return truncateToWidth(
		`${color("│")} ${truncated}${padding} ${color("│")}`,
		width,
		"",
	);
}

function estimateSystemPromptBucket(
	inputs: ContextInspectorInputs,
): ContextBucket {
	...
	const residualSystemTokens = Math.max(
		0,
		estimateTokens(inputs.systemPrompt) -
			explicitPromptInputs.reduce(
				(total, text) => total + (text ? estimateTokens(text) : 0),
				0,
			),
	);
	...
	const contributors = [
		{
			name: "Base Pi system prompt text",
			estimatedTokens: residualSystemTokens,
		},
...
function estimateToolDefinitionsBucket(
	options: BuildSystemPromptOptions,
): ContextBucket {
	const snippets = options.toolSnippets ?? {};
	const activeTools = activeToolNames(options);
	const contributors = promptVisibleToolSnippets(options).map(
...
function promptVisibleToolSnippets(
	options: BuildSystemPromptOptions,
): Array<[string, string]> {
	const snippets = options.toolSnippets ?? {};
...
function estimateContextFilesBucket(
	options: BuildSystemPromptOptions,
): ContextBucket {
	return bucket(
		"Context files",
		(options.contextFiles ?? [])
			.map((file) => contributor(file.path, file.content))
			.filter(isContributor),
	);
}

function estimateSkillsBucket(
	options: BuildSystemPromptOptions,
): ContextBucket {
	return bucket(
		"Skills",
		(options.skills ?? [])
			.map((skill) => {
				const record = skill as unknown as Record<string, unknown>;
				const name =
					typeof record.name === "string" ? record.name : "unnamed skill";
...
function estimateMessageBuckets(entries: unknown[]): ContextBucket[] {
	...
	for (const entry of entries) {
		...
		if (record.type === "branch_summary" || record.type === "compaction") {
			addGrouped(
				grouped,
				"Compactions and summaries",
				contributor(record.type, extractSummaryText(record)),
			);
			for (const retainedMessage of Array.isArray(record.retainedTail)
				? record.retainedTail
				: []) {
		...
		if (record.type === "custom_message") {
			addGrouped(
				...
				contributor(
					safeCustomMessageLabel(record.customType),
	...
	return Array.from(grouped.entries()).map(([name, contributors]) =>
		bucket(name, contributors.filter(isContributor)),
	);
}
...
function estimateAgentMessage(
	...
	if (message.role === "user") {
		addGrouped(
			grouped,
			"User messages",
			contributor("user message", extractContentText(message.content)),
		);
	} else if (message.role === "assistant") {
		addGrouped(
			grouped,
			"Assistant messages",
			contributor("assistant message", extractContentText(message.content)),
		);
		...
		addGrouped(
			...
			"Tool results",
			contributor(
				toolResultLabel(message.toolName),
		...
		addGrouped(
			...
			"Other active entries",
			contributor(
				safeCustomMessageLabel(message.customType),
				extractContentText(message.content),
			),
		);
	} else if (
		...
		addGrouped(
			grouped,
			"Compactions and summaries",
			contributor(message.role, extractSummaryText(message)),
		);
	}
}
...
function bucket(
	...
	return {
		name,
		estimatedTokens: sortedContributors.reduce(
...
function renderReadOnlyRecommendations(
	...
	if (isMaterialBucketName(buckets, "Tool results")) {
		recommendations.push(
			"Large tool results: review whether bulky outputs still need to remain in active context; a short human summary may be easier to carry forward.",
		);
	}
	if (isMaterialBucketName(buckets, "Context files")) {
		recommendations.push(
			"Large context files: consider whether the current project context can be narrowed to the files most relevant to this task.",
		);
	}
	if (isMaterialBucketName(buckets, "Skills")) {
		recommendations.push(
			"Large skills: consider whether every loaded skill is relevant to the current task before continuing.",
		);
	}
...
function isMaterialBucketName(
	buckets: Map<string, ContextBucket>,
	name: string,
): boolean {
...
function scaleTokenValues<T extends { tokens: number }>(
	...
	let remaining =
		targetTotal - scaled.reduce((total, item) => total + item.tokens, 0);
...
function renderKnownUsage(usage: ContextUsage | undefined): {
	total: string;
	percent: string;
} {

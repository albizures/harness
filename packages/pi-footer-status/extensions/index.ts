...
type FooterState = {
  ...
  entries: ExtensionContext["sessionManager"] extends { getEntries(): infer Entries } ? Entries : never;
    ...
    ctx.ui.setFooter((tui, theme, footerData) => {
      ...
      return {
        ...
        render(width: number): string[] {
          return renderFooter(
            collectFooterState(ctx, footerData, process.env.HOME || process.env.USERPROFILE),
            theme,
            width,
          );
        },
...
export function renderFooter(state: FooterState, theme: FooterTheme, width: number): string[] {
  ...
  if (usage.totals.input) statsParts.push(`↑${formatTokens(usage.totals.input)}`);
  if (usage.totals.output) statsParts.push(`↓${formatTokens(usage.totals.output)}`);
  if (usage.totals.cacheRead) statsParts.push(`R${formatTokens(usage.totals.cacheRead)}`);
  if (usage.totals.cacheWrite) statsParts.push(`W${formatTokens(usage.totals.cacheWrite)}`);
  if ((usage.totals.cacheRead > 0 || usage.totals.cacheWrite > 0) && usage.latestCacheHitRate !== undefined) {
    statsParts.push(`CH${usage.latestCacheHitRate.toFixed(1)}%`);
  }
  if (usage.totals.cost || state.usingSubscription) {
    statsParts.push(`$${usage.totals.cost.toFixed(3)}${state.usingSubscription ? " (sub)" : ""}`);
  }
  ...
  statsParts.push(colorByContextSeverity(contextPercentValue, contextPercentDisplay, theme));
  if (state.experimental) statsParts.push(`${theme.fg("dim", "•")} ${theme.bold(theme.fg("warning", "xp"))}`);

  const statsLine = renderStatsLine(statsParts.join(" "), state, theme, width);
  const contextBarLine = renderContextFillBar({ percent: contextUsage?.percent, contextWindow }, theme, width);
  ...
  const statuses = renderExtensionStatuses(state.extensionStatuses, theme, width);
  if (statuses) lines.push(statuses);
  return lines;
}

function renderStatsLine(statsLeftInput: string, state: FooterState, theme: FooterTheme, width: number): string {
  ...
  return theme.fg("dim", statsLeft) + theme.fg("dim", statsLine.slice(statsLeft.length));
}
...
export function renderContextFillBar(
  ...
  if (usage.percent === null || usage.percent === undefined) {
    ...
    return truncateToWidth(`context ${placeholder} ?/${formatTokens(usage.contextWindow)}`, width, theme.fg("dim", "..."));
  }
  ...
  const severityColor = clampedPercent > ERROR_PERCENT ? "error" : clampedPercent > WARNING_PERCENT ? "warning" : "accent";
...
function renderExtensionStatuses(statuses: ReadonlyMap<string, string>, theme: FooterTheme, width: number): string | undefined {
...
function getUsageStats(entries: FooterState["entries"]): { totals: UsageTotals; latestCacheHitRate?: number } {
...
function getEntryUsage(entry: FooterState["entries"][number]): FooterUsage | undefined {
...
function colorByContextSeverity(percent: number, text: string, theme: FooterTheme): string {

...
test("/suggested-reply inserts the selected suggestion into the editor", async () => {
  ...
  await tool.execute("call-1", { suggestions: [{ label: "First" }, { label: "Second" }] }, undefined, undefined, ctx);
...
test("function key shortcuts cycle suggestions and replace editor text", async () => {
  ...
  extension({
    ...
    registerTool(value: any) { tool = value; },
    registerCommand() {},
    registerShortcut(shortcut: string, value: any) { shortcutHandlers[shortcut] = value.handler; },
  } as any);

...
const SuggestRepliesParams = {
  ...
  properties: {
    suggestions: {
      ...
      description: "Suggested replies the user can insert into the normal prompt editor.",
      items: {
        type: "object",
        properties: {
          label: { type: "string", description: "Reply text inserted into the prompt editor." },
        },
        required: ["label"],
...
export default function suggestedReplies(pi: ExtensionAPI) {
  ...
  const showSuggestions = (ctx: Pick<ExtensionContext, "ui">, suggestions: ReplySuggestion[]) => {
    state = { suggestions: normalizeSuggestions(suggestions), selectedIndex: 0 };
    refreshWidget(ctx);
  };

  const insertSuggestion = (ctx: Pick<ExtensionContext, "ui">, index: number): boolean => {
  ...
  const cycleSuggestion = (ctx: Pick<ExtensionContext, "ui">, direction: -1 | 1): boolean => {
    if (!state || state.suggestions.length === 0) return false;
    const next = wrapIndex(state.selectedIndex + direction, state.suggestions.length);
  ...
  pi.registerTool({
    ...
    description: "Display ephemeral suggested replies that the user can insert into the normal prompt editor. Returns immediately without waiting for a selection.",
    promptSnippet: "Display suggested replies that the user can insert into the normal prompt editor",
    promptGuidelines: [
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
      ...
      if (suggestions.length === 0) {
        return { content: [{ type: "text", text: "No suggested replies displayed because no non-empty suggestions were provided." }], details: {} };
      }
      if (suggestions.length > MAX_SUGGESTIONS) {
        return { content: [{ type: "text", text: `No suggested replies displayed because at most ${MAX_SUGGESTIONS} suggestions are supported.` }], details: {} };
      }

      showSuggestions(ctx, suggestions);
      return {
        content: [{ type: "text", text: "Suggested replies displayed. The user may insert one into the prompt editor or type normally." }],
        details: { suggestions },
      };
    },
  });
...
export function normalizeSuggestions(suggestions: ReplySuggestion[]): ReplySuggestion[] {
  return suggestions
...
export function parseSuggestionNumber(args: string): number | undefined {
  ...
  if (!Number.isInteger(parsed) || parsed < 1 || parsed > MAX_SUGGESTIONS) return undefined;
  return parsed - 1;
}

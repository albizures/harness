...
When a candidate parent is supplied, inspect it with public AWF commands such as `awf get <id>` and confirm it is an AWF `kind: spec` issue before publishing child Tasks.

## Process

### 1. Gather context

Work from the existing conversation context. If the user passes a Spec id, URL, or other reference, fetch it through AWF (`awf get <id>`, `awf logs <id>`) and read the full body/history you need.
...
Publish the approved breakdown under the existing Spec with public AWF create input. Create blockers before dependents so `dependsOn` can reference real AWF ids.

Each Task input should include:
...
```json
{
  ...
  "subkind": "work",
  "dependsOn": ["<blocking task id>"],
  "generatedBy": "<source task id if this is provenance, not ordering>"
}
```

Use `parent` instead of `spec` only when the AWF command/input shape for the target parent requires it. Preserve all approved blocking edges with `dependsOn`.

Work the **frontier** via `awf ready`: any Task whose blockers and readiness gates are clear may start. Do not close or complete the parent Spec; Spec completion is a separate AWF lifecycle action after delivery.

## Boundaries

- Do not create Tasks without an existing AWF Spec parent.
- Do not use native tracker relationship APIs directly; use `spec`/`parent` and `dependsOn` in AWF input.
- Do not use private AWF runtime APIs, hidden command ids, or legacy issue-tracker conventions.
